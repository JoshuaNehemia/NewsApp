...
try {
    ...
    // 1. Check if image_path column exists
    ...
    if ($result->num_rows == 0) {
        ...
        $sql = "ALTER TABLE `news_images` 
                ADD COLUMN `image_path` VARCHAR(500) NULL AFTER `news_id`,
                ADD COLUMN `alt_text` VARCHAR(255) NULL,
                ADD COLUMN `sort_order` INT(11) DEFAULT 0;";
        
        if ($conn->query($sql) === TRUE) {
            $messages[] = "Added image_path, alt_text, sort_order columns to news_images table.";
        } else {
            throw new Exception("Error altering table: " . $conn->error);
        }
    } else {
        $messages[] = "Column image_path already exists.";
    }
